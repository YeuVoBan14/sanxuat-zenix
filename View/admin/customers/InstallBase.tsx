...
export default function InstallBase({
  ...
  const columns: ColumnDef<any>[] = [
    ...
    {
      accessorKey: "produceYear",
      header: "Năm sản xuất",
      cell: ({ row }) => <div>{row.original["produceYear"]}</div>,
    },
    {
      accessorKey: "note",
      header: "Ghi chú",
            ...
            <div className="flex gap-2 items-center">
              ...
              <div className="mt-1 mr-1">
                <AlertDialogForm
                  ...
                  title="Bạn có chắc muốn xóa máy móc này?"
                  handleSubmit={() => {}}
                />
              </div>
            </div>
  ...
  const handleExportExcel = (data: any) => {
    ...
    const heading = [
      ["Tên máy", "Loại hình", "Maker", "Số lượng", "Năm sản xuất", "Ghi chú"],
    ];
    ...
    XLSX.utils.book_append_sheet(workbook, ws, `Dữ liệu máy móc khách hàng`);
    // Save the workbook as an Excel file
    XLSX.writeFile(workbook, `may-moc-khach-hang.xlsx`);
  };
...
const formSchema: z.ZodSchema<any> = z.object({
  name: z.string().min(1, {
    message: "Bạn cần nhập tên máy móc",
  }),
  type: z.string().min(1, {
    message: "Bạn cần nhập loại máy móc",
  }),
  maker: z.string(),
  quantity: z.number(),
  produceYear: z.string(),
  note: z.string(),
});

export function CreateInstallBase({
  // id,
  edit,
  potentialState,
  setPotentialState,
...
// refetch,
{
  ...
  useEffect(() => {
    if (detailData) {
      ...
      form.setValue("maker", detailData?.maker);
      form.setValue("quantity", detailData?.quantity);
      form.setValue("produceYear", detailData?.produceYear);
    ...
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-[400px]">
          <DialogHeader>
            <DialogTitle>{`${edit ? "Cập nhật" : "Thêm"} máy móc`}</DialogTitle>
          </DialogHeader>
                        ...
                        return (
                          <FormItem className="mb-4">
                            <FormLabel className="flex">
                              <div className="text-red-600">*</div> Tên máy
                            </FormLabel>
                            <FormControl>
                              <Input
                                value={field.value}
                        ...
                        <FormItem className="mb-4">
                          ...
                          <FormControl>
                            <Input
                              value={field.value}
                              onChange={field.onChange}
                              placeholder="Nhập loại hình máy"
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
